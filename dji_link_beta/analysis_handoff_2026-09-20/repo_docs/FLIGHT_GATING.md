...
**The WM160 mode setter is the DJI Fly SoftSwitchMode request `0x06/0x59` addressed to FLYC
`0x03` (payload `0x00=Sport`, `0x01=Position/Normal`, `0x02=Tripod/Cine`).** This is easy to
miss in older midware notes because the command set is RC while the receiver is FLYC.
The live mode remains the OSD `FLYC_STATE` field; raw OSD mode-channel bits are not a wire enum.
The historical text below describes the earlier uncertainty. Older gain-parameter notes concern
speed/exposure tuning and are not the beta setter.
Evidence:
...
- Reported in telemetry: key `Flight.FlightOSD.FlightMode.Value` and `KeyFlightMode`; enum
  ...
  answer.
- Selecting Normal/Sport/Cine may also change **control-gain FC params** internally (all in `flyc_param_infos.json`,
